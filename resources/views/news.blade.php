        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                <h3 class="card-title mb-0">
                    <a href="/news/{{ $contents[0]->slug }}"
                        class="text-dark text-decoration-none card-title">{{ $contents[0]->title }}</a>
                </h3>
                <small class="text-muted">
                    By. <a href="/news?author={{ $contents[0]->author->slug }}"
                        class="text-decoration-none">{{ $contents[0]->author->name }}</a> in
                    <a href="/news?category={{ $contents[0]->category->slug }}"
                        class="text-decoration-none">{{ $contents[0]->category->name }}</a>
                    ...
                    <div class="col-md-4 mb-3">
                        <div class="card">
                            <div class="position-absolute bg-dark py-2 px-3 bg-success bg-opacity-75">
                                <a href="/news?category={{ $content->category->slug }}"
                                    class="text-decoration-none text-white">{{ $content->category->name }}</a>
                            </div>
                            @if ($content->image)
                                <div style="max-height: 400px; overflow:hidden;">
                            ...
                            <div class="card-body">
                                ...
                                <small class="card-text text-muted">
                                    By. <a href="/news?author={{ $content->author->slug }}"
                                        class="text-decoration-none">{{ $content->author->name }}</a>
                                    {{ $contents[0]->created_at->diffForHumans() }}
                                </small>
